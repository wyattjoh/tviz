...
- Paths in `infra/alchemy.run.ts` are anchored to `import.meta.dirname`, never
  ...
  rebuild.
- `public/_headers` sets `immutable` caching on `/assets/*` only. Vite fingerprints
  those; `index.html` and `public/demo/**` are not fingerprinted and must keep
  Cloudflare's revalidating default, or a redeploy strands browsers on a stale bundle
  or stale Demo Sessions. Asset requests never invoke a Worker script, so they are
  free and unlimited — keep it that way by never enabling `run_worker_first`.
...
the session log — and this project's transcripts are shared.
