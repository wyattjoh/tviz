...
describe("ContextGrid", () => {
  ...
  describe("inspecting a Cell", () => {
    ...
    it("lifts a Cell under the pointer or keyboard focus, and holds the pinned one up", () => {
      render(renderGrid(items, 200_000, ALL_SHOWN, { pinnedIndex: 1 }));

      const classesOf = (index: number): readonly string[] =>
        cells()[index]?.className.split(" ") ?? [];

      // Every Cell carries the lift for hover and focus, and honours a reduced
      // motion preference by dropping the transition rather than the lift.
      for (const state of ["hover", "focus-visible"]) {
        expect(classesOf(0)).toContain(`${state}:scale-125`);
        expect(classesOf(0)).toContain(`${state}:shadow-cell-lift/70`);
      }
      expect(classesOf(0)).toContain("motion-reduce:transition-none");
      expect(classesOf(0)).not.toContain("scale-125");

      // The pinned Cell is lifted without the pointer on it.
      expect(classesOf(1)).toContain("scale-125");
      expect(classesOf(1)).toContain("shadow-cell-lift/70");
    });
  });
});
