...
describe("Inspector", () => {
  ...
  it("caps a hovered Cell's list, and lists every item once the Cell is pinned", () => {
    const items = Array.from({ length: 15 }, (_, order) =>
      share({ category: "messages", kind: "reminder", label: `Reminder ${order + 1}`, tokens: 60 }),
    );
    const cell = cellOf("messages", items, "reminder");

    const { rerender } = render(<Inspector cell={cell} filters={ALL_SHOWN} pinned={false} />);
    // Shared rail: a dozen rows, then a count of the rest.
    expect(screen.getAllByRole("listitem")).toHaveLength(12);
    expect(screen.getByText("+3 more in this cell")).toBeDefined();

    rerender(<Inspector cell={cell} filters={ALL_SHOWN} pinned />);
    // The whole rail: every item, and nothing left to summarise.
    expect(screen.getAllByRole("listitem")).toHaveLength(15);
    expect(screen.queryByText(/more in this cell/)).toBeNull();
  });
