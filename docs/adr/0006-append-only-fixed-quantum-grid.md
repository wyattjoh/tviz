...
- The parser must emit `ContextSnapshot.items` in context order and keep it stable across calls; re-sorting items downstream would silently reintroduce the re-flow.
